...
const ROLE_BADGE = {
  WH: { label: 'WH', bg: '#dbeafe', color: '#1e40af' },
  MFG: { label: 'MFG', bg: '#dcfce7', color: '#166534' },
  LOG: { label: 'LOG', bg: '#fef3c7', color: '#92400e' },
  QA: { label: 'QA', bg: '#ede9fe', color: '#5b21b6' },
  TSF: { label: 'TSF', bg: '#e0f2fe', color: '#075985' },
  UPLOAD: { label: 'UPLOAD', bg: '#f1f5f9', color: '#475569' },
  ADMIN: { label: 'ADMIN', bg: '#fee2e2', color: '#991b1b' },
}

function RoleBadge({ role }) {
  const m = ROLE_BADGE[role] || { label: role || '—', bg: '#f1f5f9', color: '#475569' }
  return (
    <span style={{ background: m.bg, color: m.color, borderRadius: 999, padding: '2px 10px', fontSize: 12, fontWeight: 700 }}>
      {m.label}
    </span>
  )
}
...
export default function AdminDashboardPage() {
  ...
  const counts = useMemo(() => {
    const c = { total: users.length, WH: 0, MFG: 0, other: 0 }
    for (const u of users) {
      if (u.role_name === 'WH') c.WH++
      else if (u.role_name === 'MFG') c.MFG++
      else c.other++
    }
    return c
  }, [users])
  ...
  async function handleDelete(u) {
    const ok = await confirmDelete({ text: `ลบผู้ใช้ "${u.name}" ออกจากระบบ?`, confirmText: 'ลบ' })
    if (!ok) return
    try {
  ...
  return (
    <AppShell navItems={ADMIN_NAV_ITEMS} roleLabel="Admin">
      ...
      >
        {stat(<Squares2X2Icon className="size-6" />, 'ผู้ใช้ทั้งหมด', counts.total, { bg: '#eef2ff', color: '#4338ca' })}
        {stat(<RectangleStackIcon className="size-6" />, 'พนักงาน WH (คลัง)', counts.WH, { bg: '#dbeafe', color: '#1e40af' })}
        {stat(<CpuChipIcon className="size-6" />, 'พนักงาน MFG (ผลิต)', counts.MFG, { bg: '#dcfce7', color: '#166534' })}
...
function UserModal({ user, onClose, onSaved }) {
  ...
  const inputStyle = { border: '1px solid #cbd5e1', borderRadius: 8, padding: '9px 11px', fontSize: 14, width: '100%' }

  return (
    <div className="wh-modal-overlay" onClick={onClose}>
      <div className="wh-modal" style={{ maxWidth: 460 }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 10 }}>
          ...
          <div>
            <label style={labelStyle}>
              Username {isEdit && <span style={{ color: '#94a3b8' }}>(แก้ไม่ได้)</span>}
            </label>
            <input
          ...
          <div>
            ...
            <SelectField value={form.role_name} onChange={(v) => setForm((f) => ({ ...f, role_name: v }))} options={ROLE_OPTIONS} />
          </div>
          <div>
