...
export default function QAMachineList() {
  ...
  async function handleExportPDF() {
    ...
    try {
      const photoDataUrls = await Promise.all(list.map(r => r.photoURL ? fetchImageAsDataURL(`${API_BASE_URL}${r.photoURL}`) : Promise.resolve(null)));
      const doc = new jsPDF({
      ...
      const drawHeader = () => {
        ...
        doc.text(`วันที่ตรวจสอบ (Check Sheet): ${checkDateLabel}`, pageWidth - 10, 12, {
          align: 'right'
        });
  ...
  async function handleExportExcel() {
    ...
    try {
      const photos = await Promise.all(list.map(r => r.photoURL ? fetchImageForXlsx(`${API_BASE_URL}${r.photoURL}`) : Promise.resolve(null)));
      const columns = [{
        key: 'item',
        header: 'ITEM',
        type: 'number',
        ...
        key: 'exportCountry',
        header: 'ส่งออกไปประเทศ',
        type: 'center',
        width: 16
      }, {
        key: 'matchStatus',
        header: 'ผลเทียบใบอนุญาต',
        type: 'center',
        width: 16
      }, {
        key: 'confirmedAt',
  ...
  return <AppShell navItems={navItems} roleLabel="QA">
      ...
      <div className="qa-filter-card">
        ...
        <div className="qa-export-actions">
          <button className="qa-download-btn qa-export-btn" onClick={handleExportPDF} disabled={loading || filtered.length === 0 || exportingPDF} title={filtered.length === 0 ? 'ไม่มีรายการให้ออก Check Sheet' : `ดาวน์โหลด Check Sheet — ช่วง ${periodLabel}`}>
      ...
      overflowX: 'auto'
        ...
        <div className="qa-table-toolbar">
          ...
          <div className="qa-table-toolbar-right">
            <input className="wh-search" type="text" placeholder="ค้นหา Part / Machine No / IT Controller / ใบอนุญาต / อินวอยซ์..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </div>
          ...
          <tbody>
            ...
            return <tr key={r.rowKey || `${r.component}|${r.machineNo}|${r.serialNo}`}>
                    ...
                    <td data-label="รูปถ่าย">
                      {r.photoURL ? <button type="button" className="qa-photo-thumb" onClick={() => setPhotoView(r.photoURL)} title="ดูรูปถ่าย" style={{
                  padding: 0,
                  border: 'none',
                  background: 'transparent'
                }}>
                          <img src={`${API_BASE_URL}${r.photoURL}`} alt="รูปถ่ายป้าย" loading="lazy" style={{
                    width: 44,
                    height: 44,
                    objectFit: 'cover',
                    borderRadius: 6,
              ...
              <div className="wh-detail-grid">
                ...
                <div className="wh-detail-item">
                  ...
                  <span className="wh-detail-value">
                    {detailRow.checkedAtWH ? new Date(detailRow.checkedAtWH).toLocaleString('th-TH') : '—'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="wh-detail-section">
              ...
              <div className="wh-detail-grid">
                ...
                <div className="wh-detail-item">
                  <span className="wh-detail-label">เมื่อ</span>
                  <span className="wh-detail-value">
                    {detailRow.assembledAt ? new Date(detailRow.assembledAt).toLocaleString('th-TH') : '—'}
                  </span>
                </div>
              </div>
            </div>
