...
export default function LicenseAlertBanner({
  ...
  const urgent = [...expired, ...expiring].sort((a, b) => (a.DaysLeft ?? 0) - (b.DaysLeft ?? 0)).slice(0, 4);
  return <div className={`lab-banner lab-banner-${tone}`} role="alert">
      <span className="lab-banner-icon">
        {expired.length > 0 ? <ExclamationTriangleIcon className="size-5" /> : <ClockIcon className="size-5" />}
      </span>
      <div className="lab-banner-body">
        <div className="lab-banner-head">
          ...
          <div className="lab-banner-chips">
            {expired.length > 0 && <span className="lab-banner-chip lab-banner-chip-expired">
                หมดอายุแล้ว {expired.length}
              </span>}
            {expiring.length > 0 && <span className="lab-banner-chip lab-banner-chip-expiring">
        ...
        <div className="lab-banner-list">
          ...
          return <button key={`${titleOf(it)}-${idx}`} type="button" className="lab-banner-item" onClick={() => onOpenItem?.(it)} title={onOpenItem ? 'ดูในตารางด้านล่าง' : undefined}>
          ...
          {total > urgent.length && <span className="lab-banner-more">และอีก {total - urgent.length} รายการในตารางด้านล่าง</span>}
        </div>
      </div>
    </div>;
}
