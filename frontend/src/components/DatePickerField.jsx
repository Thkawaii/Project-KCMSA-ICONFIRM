...
export default function DatePickerField({
  ...
  const minKey = min ? (() => { const p = parseYMD(min); return monthKey(p.y, p.m) })() : null
  const maxKey = max ? (() => { const p = parseYMD(max); return monthKey(p.y, p.m) })() : null
  ...
  const triggerLabel = selected
    ? `${selected.d} ${THAI_MONTHS[selected.m]} ${selected.y + 543}`
    : placeholder
        ...
        <div className="dpf-pop" role="dialog">
          ...
          <div className="dpf-weekdays">
            {THAI_WEEKDAYS.map((w, i) => (
              <div key={i} className="dpf-weekday">{w}</div>
            ))}
          </div>
                ...
                return (
                  <button
                    ...
                    className={
                      'dpf-day' +
                      (inMonth ? '' : ' dpf-day-out') +
                      (isSelected ? ' dpf-day-selected' : '') +
                      (isToday && !isSelected ? ' dpf-day-today' : '')
                    }
                    disabled={disabled}
                    onClick={() => pick(d)}
                  >
