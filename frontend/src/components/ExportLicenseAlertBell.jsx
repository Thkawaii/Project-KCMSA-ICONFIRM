...
export default function ExportLicenseAlertBell() {
  ...
  const load = useCallback(async () => {
    try {
      const data = await getExportLicenseAlerts({ onlyAlert: true })
      const c = data?.counts || {}
  ...
  return (
    <div className="lab-root lab-export" ref={rootRef}>
      <button
        type="button"
        className={'lab-bell' + (badge > 0 ? ' lab-bell-active' : '') + (hasNew ? ' lab-bell-pulse' : '')}
        onClick={toggle}
        aria-label={`แจ้งเตือนอายุใบอนุญาตส่งออก${badge > 0 ? ` (${badge})` : ''}`}
        title="แจ้งเตือนอายุใบอนุญาตส่งออก"
      >
      ...
      {open && (
        <div className="lab-panel" role="dialog" aria-label="แจ้งเตือนอายุใบอนุญาตส่งออก">
          ...
          <div className="lab-list">
            ...
            {vExpired.length > 0 && (
              <>
                ...
                {vExpired.map((it) => (
                  <ExportAlertItem key={exportDismissKey(it)} item={it} onOpen={goToLicense} onDismiss={handleDismiss} />
                ))}
              </>
            )}
...
function ExportAlertItem({ item, hidden = false, onOpen, onDismiss, onRestore }) {
