...
export default function ExportLicenseAlertBell() {
  ...
  const [dismissed, setDismissed] = useState(() => readExportDismissed());
  const [showHidden, setShowHidden] = useState(false);
  const rootRef = useRef(null);
  const load = useCallback(async () => {
    try {
      const data = await getExportLicenseAlerts({
        onlyAlert: true
      });
      const c = data?.counts || {};
  ...
  function handleDismiss(item) {
    const next = addExportDismissed(item);
    setDismissed({
      ...next
    });
  ...
  function handleRestore(item) {
    const next = removeExportDismissed(item);
    setDismissed({
      ...next
    });
  }
  function handleRestoreAll() {
    setDismissed(clearExportDismissed());
    setShowHidden(false);
  }
  const badge = visibleAlert;
  const hiddenCount = hidden.length;
  const allDismissed = loaded && items.length > 0 && visibleAlert === 0 && hiddenCount > 0;
  return <div className="lab-root lab-export" ref={rootRef}>
      <button type="button" className={'lab-bell' + (badge > 0 ? ' lab-bell-active' : '') + (hasNew ? ' lab-bell-pulse' : '')} onClick={toggle} aria-label={`แจ้งเตือนอายุใบอนุญาตส่งออก${badge > 0 ? ` (${badge})` : ''}`} title="แจ้งเตือนอายุใบอนุญาตส่งออก">
      ...
      {open && <div className="lab-panel" role="dialog" aria-label="แจ้งเตือนอายุใบอนุญาตส่งออก">
          ...
          <div className="lab-list">
            ...
            {loaded && badge === 0 && !allDismissed && <div className="lab-empty lab-empty-ok">
                ...
                ทุกใบอนุญาตส่งออกยังอยู่ในอายุ ไม่มีรายการต้องจัดการ
              </div>}
            ...
            {vExpired.length > 0 && <>
                ...
                {vExpired.map(it => <ExportAlertItem key={exportDismissKey(it)} item={it} onOpen={goToLicense} onDismiss={handleDismiss} />)}
              </>}
            ...
            {showHidden && hidden.length > 0 && <>
                ...
                {hidden.map(it => <ExportAlertItem key={exportDismissKey(it)} item={it} hidden onOpen={goToLicense} onRestore={handleRestore} />)}
              </>}
          </div>
