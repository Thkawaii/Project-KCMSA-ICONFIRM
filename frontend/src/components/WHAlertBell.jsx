...
export default function WHAlertBell() {
  ...
  const load = useCallback(async () => {
    const [imp, exp] = await Promise.allSettled([getImportLicenseAlerts({
      onlyAlert: true
    }), getExportLicenseAlerts({
      onlyAlert: true
    })]);
    let impList = [];
    let expList = [];
  ...
  const goImport = item => {
    setOpen(false);
    navigate('/warehouse', item ? {
      focusLicense: item.LicenseNo || '',
      focusInvoice: item.InvoiceNo || '',
      focusTs: Date.now()
    } : {});
  };
  const goExport = item => {
  ...
  return <div className="lab-root" ref={rootRef}>
      <button type="button" className={'lab-bell' + (badge > 0 ? ' lab-bell-active' : '') + (hasNew ? ' lab-bell-pulse' : '')} onClick={toggle} aria-label={`แจ้งเตือนอายุใบอนุญาต${badge > 0 ? ` (${badge})` : ''}`} title="แจ้งเตือนอายุใบอนุญาต (นำเข้า + ส่งออก)">
      ...
      {open && <div className="lab-panel lab-panel-wide" role="dialog" aria-label="แจ้งเตือนอายุใบอนุญาต">
          ...
          {loaded && totalOutstanding === 0 && <div className="lab-empty lab-empty-ok" style={{
        margin: 12
      }}>
          ...
          <AlertSection theme="import" title="ใบอนุญาตนำเข้า" kindIcon={<ArrowDownTrayIcon className="size-3" />} loaded={loaded} expired={imp.expired} expiring={imp.expiring} limit={impLimit} onLimitChange={setImpLimit} noDate={impNoDate} noDateLabel="ยังไม่ได้ระบุวันที่ออกใบอนุญาต" expiringLabel="ใกล้หมดอายุ (ภายใน 30 วัน)" onOpen={goImport} getKey={impKey} renderMeta={it => <>
...
function AlertSection({
  ...
  return <div className={'lab-section' + themeClass}>
      ...
      {total > 0 && <div className="lab-limit-filter" role="group" aria-label={`แสดงล่าสุดกี่รายการ (${title})`}>
          <span className="lab-limit-label">ล่าสุด</span>
          {LIMIT_OPTIONS.map(o => <button key={String(o.value)} type="button" className={'lab-limit-btn' + (limit === o.value ? ' is-active' : '')} onClick={() => onLimitChange(o.value)}>
              {o.label}
            </button>)}
        </div>}

      <div className="lab-list">
        {loaded && total === 0 && <div className="lab-empty lab-empty-ok">
            <span className="lab-empty-dot" />
        ...
        {expiredShown.length > 0 && <>
            ...
            {expiredShown.map(it => <Row key={getKey(it)} item={it} onOpen={onOpen} renderMeta={renderMeta} titleField={titleField} />)}
          </>}
