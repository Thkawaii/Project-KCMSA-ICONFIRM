...
import { ShieldCheckIcon, XMarkIcon, ArrowDownTrayIcon, ArrowUpTrayIcon, ChevronRightIcon } from './icons.jsx';
...
export default function LicenseWeeklyPopup() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!alive) return;
      const impList = (imp.status === 'fulfilled' ? imp.value?.items || [] : []).filter(isAlert).map(it => ({
        ...it,
        kind: 'import'
      }));
      const expList = (exp.status === 'fulfilled' ? exp.value?.items || [] : []).filter(isAlert).map(it => ({
        ...it,
        kind: 'export'
      }));
  ...
  return <div className={'lwp-overlay' + (closing ? ' is-closing' : '')} role="presentation" onClick={handleClose}>
      <div className={'lwp-card' + (closing ? ' is-closing' : '')} role="dialog" aria-modal="true" aria-labelledby="lwp-title" onClick={e => e.stopPropagation()}>
        ...
        <div className="lwp-body">
          ...
          <div className="lwp-list">
            ...
            return <button key={`${it.kind}-${titleOf(it)}-${idx}`} type="button" className="lwp-item" onClick={() => openItem(it)} title="ดูในหน้าจัดการใบอนุญาต">
                  <span className={'lwp-item-icon lwp-item-icon-' + it.kind}>
                    {it.kind === 'import' ? <ArrowDownTrayIcon className="size-4" /> : <ArrowUpTrayIcon className="size-4" />}
                  </span>
                  ...
                  <span className="lwp-item-right">
                    <span className={'lwp-item-days ' + (isExp ? 'is-expired' : 'is-expiring')}>
          ...
          {total > rows.length && <p className="lwp-more">
